...
Benchmark paths (always run in order):
  ...
  D: Custom Chain                   — CustomVLAChain: vision SDPA + LLM custom Triton + custom action kernels
...
def main():
    ...
    try:
        ...
        # 1. Build unified chain (VLM + ActionModel + Memory). Vision attention is
        #    SDPA; LLM attention is the custom Triton kernel; memory runs the custom
        #    Triton chain (build_custom_vla_chain wires CustomMemoryChain natively
        #    without mutating gs_vla itself).
