...
Benchmark paths (always run in order):
  ...
  D: Custom Chain                      — vision attention = SDPA (fixed); LLM attention = custom Triton; + torch.compile
...
def main():
    ...
    # =========================================================================
    # Path D: CustomVLMChain — vision SDPA + LLM custom
    # =========================================================================
    print(f"\n{'=' * 60}")
    print("Path D: CustomVLMChain (vision SDPA, LLM custom)")
    print(f"{'=' * 60}")
    try:
        ...
        # Vision attention is SDPA, LLM attention is the custom Triton kernel (both fixed).
        ...
        build_times["D: vision-SDPA + LLM-custom"] = chain_compile_time

        def custom_fn():
            with torch.no_grad():
                return compiled_chain(pv)

        run_benchmark("D: vision-SDPA + LLM-custom", custom_fn)
        torch._dynamo.reset()
    except Exception as e:
